...
actions+=/bristling_fur,if=buff.ironfur.stack<=1
actions+=/ironfur,if=!buff.ironfur.up|buff.gory_fur.up|rage>=80
actions+=/thrash_bear,if=spell_targets.thrash_bear>=2
actions+=/mangle
actions+=/moonfire,if=buff.galactic_guardian.up
actions+=/thrash_bear
actions+=/pulverize,if=(buff.pulverize.remains<=6&dot.thrash_bear.stack>=2)|dot.thrash_bear.stack>3+equipped.elizes_everlasting_encasement*2
actions+=/moonfire,target_if=dot.moonfire.refreshable&(spell_targets.swipe_bear<=3|equipped.fury_of_nature)
actions+=/swipe_bear
]])
