...
local SpellData = LibStub("LibSpellData-5.0")
...
function TJ:DetectAbilitiesFromSpellBook()
    ...
    local function RetrieveSpells(bookType, caster)
        local st = CT()
        for spellID, spellName, spellSubText, spellBookSlotID, spellIsTalent, spellIcon, castTime, isPassive, isOffSpec, isPlayerSpell in SpellData.IterateSpellBook(bookType, st) do
    ...
    definedAbilities = TJ:MergeTables(abilities, definedAbilities)
...
function TJ:ShowSpellExportWindow()
    ...
    for k,v in TJ:OrderedPairs(definedAbilities) do
        if not tContains(blacklistedExportedAbilities, k) then
            local line = TJ:Format('    %s = { ', k)
            if v.KeyedSpellIDs then
                local ids = {}
                for id in TJ:OrderedPairs(v.KeyedSpellIDs) do
                    ids[1+#ids] = id
                end
                line = line .. TJ:Format('SpellIDs = { %s }, ', tconcat(ids, ", "))
            end
            if v.TalentID then line = line .. TJ:Format('TalentID = %d, ', v.TalentID) end
            line = line .. '},'
            addline(line)
        end
    end
    ...
    TJ:OpenDebugWindow('Thousand Jabs Actions Data Export', export)
end

TJ:RegisterCommandHandler('_esd', 'Exports spell data', function() TJ:ExportAbilitiesFromSpellBook() end)
...
function TJ:ExportSpecsTables()
    ...
    for className,classData in TJ:OrderedPairs(allSpecs) do
        output = output .. TJ:Format("%s\n    %s => {", firstClass and '' or ',', className)
        output = output .. TJ:Format("\n        classID => %d,", classData.classID)
        output = output .. TJ:Format("\n        name => '%s',", classData.className)
        output = output .. TJ:Format("\n        specs => {")
        local firstSpec = true
        for specName,specData in TJ:OrderedPairs(classData.specs) do
    ...
    TJ:OpenDebugWindow('Thousand Jabs Spec Data Export', output)
end
