...
sub create_action_lists {
    ...
    for my $cls (sort keys %{$profiles}) {
        ...
        my $ucls                   = uc $cls;
        my $class_lua_actions_file = "${cfg::script_dir}/TJ5/Class_${cls}/Generated-Actions.lua";
        my $bn                     = basename($class_lua_actions_file);

        if(!-d "${cfg::script_dir}/TJ5/Class_${cls}") {
            mkdir("${cfg::script_dir}/TJ5/Class_${cls}");
        }
    ...
    for my $cls (sort keys %{$customprofiles}) {
        ...
        for my $spec (@{ $customprofiles->{$cls} }) {
            ...
            print {$outfile} "Engine:RegisterActionProfileList('custom::${lcls}::${spec}', 'Thousand Jabs Custom ${className} Profile: ${specName}', ${classID}, ${specID}, [[\n";
    ...
    for my $cls (sort keys %{$profiles}) {
        ...
        for my $spec (sort keys %{ $profiles->{$cls} }) {
            ...
            print {$outfile} "Engine:RegisterActionProfileList('simc::${lcls}::${spec}', 'Simulationcraft ${className} Profile: ${specName}', ${classID}, ${specID}, [[\n";
