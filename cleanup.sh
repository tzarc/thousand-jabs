...
tjfind() {
    find . \( "$@" \) -and -not -path './.git/*' -and -not -path './simc*' -and -not -path './Temp*' -and -not -path '*/DataGenerator/build*' -and -not -path '*/DataGenerator/3rdparty*' -and -not -path '*/DataGenerator/generated*' -print | sort
}
