...
tjfind() {
    find . \( "$@" \) -and -not -path './.git/*' -and -not -path './simc*' -and -not -path './Temp*' -and -not -path './Support/DataGenerator/build*' -and -not -path './Support/DataGenerator/3rdparty*' -and -not -path './Support/DataGenerator/generated*' -print | sort
}
