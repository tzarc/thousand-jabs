...
function TJ:ExecuteAllActionProfiles()
    local ok, err = pcall(function()
        ...
        UI:SetAction(UI.SINGLE_TARGET, 1, self.st_state.env[action].Icon)
        action = self.st_state:PredictActionFollowing(action) or "wait"
        UI:SetAction(UI.SINGLE_TARGET, 2, self.st_state.env[action].Icon)
        action = self.st_state:PredictActionFollowing(action) or "wait"
        UI:SetAction(UI.SINGLE_TARGET, 3, self.st_state.env[action].Icon)
        action = self.st_state:PredictActionFollowing(action) or "wait"
        UI:SetAction(UI.SINGLE_TARGET, 4, self.st_state.env[action].Icon)
        ...
        if Config:Get('displayMode') ~= 'automatic' then
            if Config:Get('showCleave') then
                ...
                UI:SetAction(UI.CLEAVE, 1, self.cleave_state.env[action].Icon)
                action = self.cleave_state:PredictActionFollowing(action) or "wait"
                UI:SetAction(UI.CLEAVE, 2, self.cleave_state.env[action].Icon)
            end
            ...
            if Config:Get('showAoE') then
                ...
                UI:SetAction(UI.AOE, 1, self.aoe_state.env[action].Icon)
                action = self.aoe_state:PredictActionFollowing(action) or "wait"
                UI:SetAction(UI.AOE, 2, self.aoe_state.env[action].Icon)
            end
        end
    end)
