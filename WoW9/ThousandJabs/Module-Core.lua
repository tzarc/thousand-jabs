...
local fake_G =
	...
	{},
	{
		__index = function(tbl, key)
			...
			if tContains(globalExcludes, key) then
				return val
			end
			if not Core.devMode and val ~= nil then
				return val
			end
			...
			if Core.devMode and val ~= nil then
				return val
			end
			return nil
		end,
...
function Core:DevPrint(...)
	if Core.devMode then
		Core:Print("%.3f: %s", debugprofilestop(), Core:Format(...))
	end
end
...
function Core:Error(fulltxt)
	if not tContains(otherErrors, fulltxt) then
		...
		if not Core.errorThrown then
			...
			print(
				"|cFFFF9900Please raise a ticket on the project page on curseforge, and paste the output from the command: |cFFFFFF00/tj ticket|r"
			)
		end
	end
...
local function targetFieldName(tableName, key)
	return type(key) == "number" and ("%s[%d]"):format(tableName, tostring(key)) or
		("%s.%s"):format(tableName, tostring(key))
end
...
function Core:Debug(...)
	if disableDebugOutput then
		return
	end
	if Config:Get("do_debug") then
		if #debugLines == 0 then
			debugLines[1] = Core:Format("|cFFFFFFFFThousandJabs Debug log|r (|cFF00FFFFhide with /tj _dbg|r):")
		end
		local a = ...
		...
		if (InCombatLockdown() and Broker.updateTime < 30) or (Broker.updateTime < 100) then -- calc in-combat if <30ms, or out-of-combat if <100ms
			...
			if Stats.lastCheckDelta then
				...
				if Stats.lastCpuAmount and Stats.lastCpuAmount > 0 then
					...
					Broker.dataObj.text =
						Broker.dataObj.text .. Core:Format(", CPU: %.1f%% (%.3fms)", 100 * (delta / dt) / 1000.0, delta / dt)
				end
			end
		else
			Broker.dataObj.text =
				Core:Format("Thousand Jabs: Statistics disabled, too much time used (%d ms)", Broker.updateTime)
		end
	end
end
...
local function tierSelections()
	...
	for k = 1, 7 do
		talents[k] =
			select(10, GetTalentInfoBySpecialization(specID, k, 1)) and 1 or
			select(10, GetTalentInfoBySpecialization(specID, k, 2)) and 2 or
			select(10, GetTalentInfoBySpecialization(specID, k, 3)) and 3 or
			999
	end
...
local function equippedItems()
	...
	for _, slot in pairs(inventorySlots) do
		...
		if ok and inventoryID then
			slotlinks[slot] = (GetInventoryItemLink("player", inventoryID) or ""):gsub("|", "||")
		end
	end
	return slotlinks
end
...
function Core:ExportDebuggingInformation()
	if InCombatLockdown() then
		...
		Core:OpenDebugWindow(
			"ThousandJabs Diagnostic Information",
			Core:Format("ThousandJabs Diagnostic Information:\n%s", LSD(export))
		)
	end
end
