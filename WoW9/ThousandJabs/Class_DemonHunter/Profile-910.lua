...
local vengeance_base_overrides = {
	...
	infernal_strike = {
		...
		aura_duration = function(spell, env)
			return env.abyssal_strike.talent_enabled and env.sigil_of_flame.aura_duration or 0
		end,
		spell_delay = function(spell, env)
			return env.abyssal_strike.talent_enabled and env.sigil_of_flame.spell_delay or 0
		end,
		aura_remains = function(spell, env)
			return env.abyssal_strike.talent_enabled and mmax(0, spell.aura_duration - spell.time_since_last_cast or 0)
		end,
		spell_cast_time = 0.01, -- off GCD!
	...
	shear = {
		...
		HistoricalCast = function(spell, env, state, originalCastTime)
			...
			if delta < spell.fragment_delay then
				state:Defer(originalCastTime + spell.fragment_delay, "fracture", "add_fragments", 1)
			end
		end,
...
local vengeance_talent_overrides = {
	...
	fracture = {
		...
		HistoricalCast = function(spell, env, state, originalCastTime)
			local delta = env.currentTime - originalCastTime
			if delta < spell.fragment_delay then
				state:Defer(originalCastTime + spell.fragment_delay, "fracture", "add_fragments", 2)
			end
		end,
		Deferred = function(spell, env, state, triggerTime, ...)
			local cmd, amount = ...
...
local function sigilInitialiser(duration)
	return {
		spell_delay = function(spell, env)
			return env.quickened_sigils.talent_enabled and 1 or 2
		end,
		aura_duration = function(spell, env)
			return 8 + spell.spell_delay
		end,
		placed = function(spell, env)
			return spell.time_since_last_cast < spell.aura_duration
		end,
		aura_remains = function(spell, env)
			return spell.aura_duration - spell.time_since_last_cast
		end -- TODO: Is this how long the DoT has to go?
	}
end
...
local vengeance_sigil_overrides = {
	any_sigil = {
		placed = function(spell, env)
			return env.sigil_of_flame.placed or env.sigil_of_misery.placed or env.sigil_of_silence.placed or
				(env.sigil_of_chains.talent_enabled and env.sigil_of_chains.placed) or
				(env.abyssal_strike.talent_enabled and env.infernal_strike.time_since_last_cast < env.infernal_strike.aura_duration)
		end
	},
	spell_any_flame_sigil = {
		placed = function(spell, env)
			return env.sigil_of_flame.placed or
				(env.abyssal_strike.talent_enabled and env.infernal_strike.time_since_last_cast < env.infernal_strike.aura_duration)
		end
	},
