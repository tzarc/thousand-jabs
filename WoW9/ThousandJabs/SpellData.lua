...
local function slug(name)
	if GetLocale() ~= "enUS" then
		return name
	end -- Slug names are only relevant for en-US, other locales are internally matched by spellID anyway
...
function TJ:DetectAbilitiesFromSpellBook()
	...
	local function RetrieveSpells(bookType, caster)
		local st = ct()
		for spellID, spellName, spellSubText, spellBookSlotID, spellIsTalent, spellIcon, castTime, isPassive, isOffSpec, isPlayerSpell in SpellData.IterateSpellBook(
			bookType
		) do
...
function TJ:ExportAbilitiesFromSpellBook(runAllPossibleCombinations)
	...
	for k = 1, GetMaxTalentTier() do
		talents[k] =
			...
			select(10, GetTalentInfoBySpecialization(specID, k, 3)) and 3
	end
...
function TJ:ShowSpellExportWindow()
	...
	for k, v in Core:OrderedPairs(definedAbilities) do
		if not tContains(blacklistedExportedAbilities, k) then
			...
			if v.TalentID then
				line = line .. Core:Format("TalentID = %d, ", v.TalentID)
			end
			line = line .. "},"
			addline(line)
		end
		...
		if className then
			...
			for specID = 1, 10 do
				...
				if specNumber then
					...
					allSpecs[noSpaceClassName] =
						allSpecs[noSpaceClassName] or
						{
							classID = classID,
							className = className,
