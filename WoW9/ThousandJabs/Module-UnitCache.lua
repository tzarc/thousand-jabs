...
local function GetAuraValues(unit, idx, filter)
	...
	v.name,
		...
		v.value1,
		v.value2,
		v.value3 = UnitAura(unit, idx, filter, filter)
	v.unit = unit
...
function UnitCache:UpdateUnitCache(unitID, forceUpdate)
	if UnitExists(unitID) then
		...
		if forceUpdate or not unitCache[theGUID] then
			...
			if unitCache[theGUID].auras then
				rt(unitCache[theGUID].auras)
			end
...
function UnitCache:COMBAT_LOG_EVENT_UNFILTERED()
	local timeStamp,
		...
		V13,
		V14,
		V15 = CombatLogGetCurrentEventInfo()
	if
		(sourceGUID == playerGUID or sourceGUID == targetGUID or destGUID == playerGUID or destGUID == targetGUID) and
			combatEvent:sub(1, 11) == "SPELL_AURA_"
	 then
		forceUpdate = true
	end
...
updateFrame:SetScript(
	...
	function(self, eventName, ...)
		UnitCache[eventName](UnitCache, eventName, ...)
	end
)
