...
local function CreatePrevGcdTable(state, profile)
	...
	for i = 1, 10 do
		prev_gcd[i] =
			setmetatable({}, {__gcd = i, __env = state.env, __profile = profile, __index = PrevGcdTableIndexerPrototype})
	end
	return prev_gcd
end
...
local function StateResetPrototype(self, targetCount, seenTargets)
	...
	for k, v in pairs(env) do
		if type(v) == "table" then
			...
			if rawget(entry, "AuraID") then
				...
				if aura and v.expirationTime == 0 then
					v.expirationTime = GetTime() + 99999
				end -- Handle no-timer auras that are present
	...
	local pInterruptible =
		(pName and not (select(8, UnitCastingInfo("player")) or select(7, UnitChannelInfo("player")))) and true or false
	local tName = (UnitCastingInfo("target") or UnitChannelInfo("target"))
	local tInterruptible =
		(tName and not (select(8, UnitCastingInfo("target")) or select(7, UnitChannelInfo("target")))) and true or false
	local mName = (UnitCastingInfo("pet") or UnitChannelInfo("pet"))
	local mInterruptible =
		(mName and not (select(8, UnitCastingInfo("pet")) or select(7, UnitChannelInfo("pet")))) and true or false
...
local function StateCastActionPrototype(self, action)
	...
	if act.AbilityID and act.AbilityID ~= 61304 then -- Don't count the 'wait' ability
		...
		local t = ct()
		t.time, t.ability, t.caster, t.offGCD =
			env.currentTime,
			act.ActionName,
			act.SpellBookCaster,
			((act.spell_cast_time < 0.1) and true or false)
...
local function StatePredictActionAtOffsetPrototype(self, predictionOffset, performPostCastSpellID)
	...
	if not env.in_combat then
		return self:ExecuteActionProfileList("precombat") or self:ExecuteActionProfileList("default") or nil
	end
		...
		if not tContains(self.profile.blacklisted, action.action) then
			...
			if action.params.debug then
				if not action.keywords_printer then
					local entries =
						(action.action == "variable" or action.action == "call_action_list" or action.action == "run_action_list") and {} or
						{
							Core:Format(
								"\n'|cFFFFFF99%s.spell_can_cast=|cFFFF9900' .. tostring(%s.spell_can_cast)",
								action.action,
								action.action
							)
						}
					local keywords =
						action.params.condition_converted and action.params.condition_converted.keywords or
						action.params.value_converted and action.params.value_converted.keywords or
						action.params.target_if_converted and action.params.target_if_converted.keywords or
						{}
					for k, v in pairs(keywords) do
						entries[1 + #entries] =
							Core:Format(
							"'|cFFFFFF99%s=|cFFFF9900' .. (type(%s) == 'number' and ('%%.2f'):format(%s) or tostring(%s))",
							v,
							v,
							v,
							v
			...
			if action.action == "variable" then
				...
				if not status then
					...
					Core:Error(
						Core:Format("Error executing variable function:\n------\n%s\n------\n%s\n------", ret, action.fullvaluefuncsrc)
					)
				else
					-- Update the value
					env.variable[action.params.name] = ret
					Core:Debug(
						"|cFFFF99FF%s ==> |cFFDD00FF%s = %s|cFFFF99FF: %s|r",
						action.key,
						action.params.name,
						tostring(ret),
						action.fullvaluefuncsrc
					)
				end
			elseif action.condition_func then -- We have a valid check function
				...
				if not status then
					...
					Core:Error(
						Core:Format(
							"Error executing condition function:\n------\n%s\n------\n%s\n------",
							ret,
							action.fullconditionfuncsrc
						)
					)
				...
				elseif ret == true then
					...
					if action.action == "call_action_list" or action.action == "run_action_list" then
						...
						Core:Debug(
							"|cFF99FFFF%s ==> '|cFF00DDFF%s|cFF99FFFF': %s|r",
							action.key,
							action.params.name,
							action.fullconditionfuncsrc
						)
						...
						Core:Debug(
							"|cFF99FF99%s ==> '|cFF00FF00%s|cFF99FF99': %s|r",
							action.key,
							action.action,
							action.fullconditionfuncsrc
						)
