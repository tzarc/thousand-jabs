...
local function HandleUnitHostileActionEvent(
	combatEvent,
	sourceGUID,
	sourceName,
	sourceFlags,
...
function TJ:COMBAT_LOG_EVENT_UNFILTERED(eventName)
	...
	local timeStamp,
		...
		arg22,
		arg23,
		arg24 = CombatLogGetCurrentEventInfo()
	...
	if sourceName and sourceGUID and sourceName ~= "" and destName and destGUID and destName ~= "" then
		...
		if UnitHostileActionEvents[combatEvent] then
			local hostileAdded, hostileGUID, hostileName, hostileFlags =
				HandleUnitHostileActionEvent(combatEvent, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags)
		...
		if UnitDespawnEvents[combatEvent] then
			local hostileRemoved, hostileGUID, hostileName, hostileFlags =
				HandleUnitDespawnEvent(combatEvent, destGUID, destName, destFlags)
