...
function TJ:RunFuncCoroutines()
	...
	local exists = false
	for _, _ in pairs(commandQueue) do
		exists = true
	end
	if exists then
		TJ:QueueUpdate()
...
function TJ:GetActiveProfile()
	...
	local profile =
		(not isDisabled) and (self.profiles and self.profiles[classID] and self.profiles[classID][specID]) or nil
	return profile
end
...
function TJ:ExecuteAllActionProfiles()
	...
	Core:Debug("")
	Core:Debug(
		Config:Get("displayMode") ~= "automatic" and "|cFFFFFFFFSingle Target|r" or "|cFFFFFFFFAutomatic Target Counting|r"
	)
...
function TJ:GetIncomingDamage(timestamp, secs)
	...
	for entrytime, damage in pairs(self.damageTable) do
		...
		if entrytime < now - 60 then
			toDelete[1 + #toDelete] = entrytime
		end
	...
	-- Perform deletes
	for i = 1, #toDelete do
		self.damageTable[toDelete[i]] = nil
	end
	rt(toDelete)
	return value
end
...
function TJ:ConsoleCommand(args)
	...
	elseif argv[1] == "_dtc" then
		...
		Core:Print(
			" - Total allocated: %d, total acquired: %d, total released: %d, total in-use: %d",
			TableCache.TableCache.TotalAllocated,
			TableCache.TableCache.TotalAcquired,
			TableCache.TableCache.TotalReleased,
			TableCache.TableCache.TotalAcquired - TableCache.TableCache.TotalReleased
		)
	...
	elseif argv[1] == "_dcp" then
		...
		Core:Print(
			"     |cFFFF6600/tj ticket|r - Shows a window that can be used to copy/paste debugging information for raising tickets."
		)
