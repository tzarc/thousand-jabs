...
do
	removeLNS = function(txt)
		return type(txt) == "string" and LARGE_NUMBER_SEPERATOR:len() > 0 and
			txt:gsub(RegexPatterns[LARGE_NUMBER_SEPERATOR], "") or
			txt
	end
		...
		if state.BookType == BOOKTYPE_SPELL then
			...
			if state.Mode == "SPELL" then -- In "SPELL" mode, iterate over the slots
				...
				if state.SpellBookIndex > state.TabLastIndex then
					...
					if state.Tab > GetNumSpellTabs() then
						return nil
					end
...
do
	...
	tts:AddFontStrings(
		tts:CreateFontString("$parentTextLeft1", nil, "GameTooltipText"),
		tts:CreateFontString("$parentTextRight1", nil, "GameTooltipText")
	)
	...
	local function tooltipIteratorDispatch(state)
		...
		if state.Line > tts:NumLines() then
			return nil
		end
	...
	lib.IterateSpellTooltip = function(spellID, tstate)
		return IterateLinkTooltip(("spell:%d"):format(spellID), tstate)
	end
end
		...
		for _, s in pairs(PowerSuffixes) do
			...
			if _G[b] then
				...
				t =
					t:gsub(
					"|4([^:]+):([^;]+);",
					function(a, b) -- (a or b - "|4aaa:bbb;")
						A, B = a, b
						return placeholder
					end
				)
				if A then
					PowerPatterns[1 + #PowerPatterns] = {
						name = b:gsub("_COST", ""):lower(),
						regex = "^" .. t:gsub(placeholder, A) .. "$"
					}
					PowerPatterns[1 + #PowerPatterns] = {
						name = b:gsub("_COST", ""):lower(),
						regex = "^" .. t:gsub(placeholder, B) .. "$"
					}
				else
...
do
	local InternalMemoryTest = function()
		...
		for i = 1, iterations do
			for spellID, spellName, spellSubText, spellBookSlotID, spellIsTalent, spellIcon, castTime, isPassive, isOffSpec, isPlayerSpell in LSD.IterateSpellBook(
				BOOKTYPE_SPELL
			) do
		...
		print(
			("Time taken each run: %.3f msecs, Memory usage delta: %d bytes"):format(
				(afterTime - beforeTime) / iterations,
...
local function LibSpellDataExample()
	...
	for spellID, spellName, spellSubText, spellBookSlotID, spellIsTalent, spellIcon, castTime, isPassive, isOffSpec, isPlayerSpell in LSD.IterateSpellBook(
		...
		if spellBookSlotID ~= nil and not isOffSpec then
			...
			local rc, rch = LSD.GetSpellRechargeTime(spellID)
			print(
				("%7d -- |T%d:0|t %-40s -- slotID: %3d -- cost: %s %s %s %s -- cooldown: %s %s -- recharge: %s %s"):format(
					spellID,
					spellIcon,
					spellName,
					spellBookSlotID,
