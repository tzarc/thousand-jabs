...
function Profiling:ProfilingEpilog(...)
	...
	if not self.profiling.enabled or #self.profiling.stack == 0 then
		return
	end
	...
	for k, v in Core:OrderedPairs(self.profiling.data) do
		if type(v) == "table" then
			l[1 + #l] =
				do_mem and
				Core:Format(
					"%5dx %9.3fms/ea, %13.3fms/tot: %-40s | mem=%11s bytes/ea, %15s bytes/tot",
					v.count,
					v.timeSpent / v.count,
					v.timeSpent,
					k,
					FormatLargeNumber(mfloor(1024 * v.memGain / v.count)),
					FormatLargeNumber(mfloor(1024 * v.memGain))
				) or
				Core:Format("%5dx %6.3fms/ea, %10.3fms/tot: %s", v.count, v.timeSpent / v.count, v.timeSpent, k)
		end
	end
	local s = tconcat(l, "\n  ")
	rt(l)
