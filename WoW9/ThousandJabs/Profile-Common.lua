...
Core.Environment.common = {
	...
	wait = {
		...
		spell_cast_time = 1,
		perform_cast = function(spell, env)
		end
	},
	...
	race = {
		blood_elf = function(self, state)
			return select(2, UnitRace("player")) == "BloodElf" and true or false
		end
	},
	raid_movement = {
		remains = 0,
	...
	level = {
		curr = function(self, state)
			return UnitLevel("player") or 0
		end
	},
	health = {
		curr = function(self, state)
			return self.sampled + self.gained - self.lost
		end,
		gained = 0,
		lost = 0,
		sampled = function(self, state)
			return UnitHealth("player") or 0
		end,
		max = function(self, state)
			return UnitHealthMax("player") or 0
		end,
		percent = function(self, state)
			return UnitExists("player") and (100 * self.curr / UnitHealthMax("player")) or 0, true
		end,
		target_percent = function(self, state)
			return UnitExists("target") and (100 * UnitHealth("target") / UnitHealthMax("target")) or 0, true
		end
	},
	target = {
		curr_health = function(self, state)
			return UnitHealth("target") or 0
		end,
		exists = function(self, state)
			return UnitExists("target"), true
		end,
		hostile = function(self, state)
			return self.exists and (UnitReaction("target", "player") or 4) <= 4
		end,
		time_to_die = function(self, state)
			return UnitCache:UnitTimeToDie("target") or 99999
		end,
		is_casting = false,
	...
	adds = {
		raid_event_exists = function(self, state)
			return (state.active_enemies > 1) and true or false
		end,
		raid_event_up = function(self, state)
			return self.raid_event_exists
		end,
		raid_event_in = 180,
	...
	-- Covenants
	covenant = {
		kyrian = function(spell, env)
			return (C_Covenants_GetActiveCovenantID() == 1) and true or false
		end,
		venthyr = function(spell, env)
			return (C_Covenants_GetActiveCovenantID() == 2) and true or false
		end,
		night_fae = function(spell, env)
			return (C_Covenants_GetActiveCovenantID() == 3) and true or false
		end,
		necrolord = function(spell, env)
			return (C_Covenants_GetActiveCovenantID() == 4) and true or false
		end
	},
	kyrian = {
		covenant_enabled = function(spell, env)
			return (C_Covenants_GetActiveCovenantID() == 1) and true or false
		end
	},
	venthyr = {
...
local function rune_calc(currentTime, rune_tbl, gained, spent)
	...
	for i = 1, gained do
		rune_tbl[i] = 0
	end -- Change to zero at the front to cater for gained runes
	for i = 1, spent do
		rune_tbl[i] = 15
	end -- Change to 15 at the front to cater for spent runes
	tsort(rune_tbl)
	return rune_tbl
end
...
Core.Environment.resources = {
	mana = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Mana) or 0)
		end,
		regen = function(power, env)
			return GetPowerRegen()
		end,
		spent = 0,
		gained = 0,
		curr = function(power, env)
			return power.sampled + power.gained - power.spent + power.regen * env.predictionOffset
		end,
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Mana) or 0)
		end,
		time_to_max = function(power, env)
			return (power.max - power.curr) / power.regen
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	soul_shards = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_SoulShards) or 0)
		end,
		spent = 0,
		gained = 0,
		curr = function(power, env)
			return power.sampled - power.spent + power.gained
		end,
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_SoulShards) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	energy = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Energy) or 0)
		end,
		regen = function(power, env)
			return GetPowerRegen()
		end,
		...
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Energy) or 0)
		end,
		time_to_max = function(power, env)
			return (power.max - power.curr) / power.regen
		end,
	...
	chi = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Chi) or 0)
		end,
		spent = 0,
		gained = 0,
		curr = function(power, env)
		...
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Chi) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	rage = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Rage) or 0)
		end,
		spent = 0,
		gained = 0,
		gained_from_autoattacks = function(spell, env)
			if IsArmsWarrior() then
				...
				if TJ.lastMainhandAttack < now - swingspeed then
					TJ.lastMainhandAttack = now
				end
		...
		curr = function(power, env)
			return power.sampled + power.gained + power.gained_from_autoattacks - power.spent
		end,
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Rage) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	pain = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Pain) or 0)
		end,
		gained = 0,
		spent = 0,
		curr = function(power, env)
		...
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Pain) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	fury = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Fury) or 0)
		end,
		spent = 0,
		gained = 0,
		gained_from_autoattacks = function(spell, env)
			if IsHavoc() and env.demon_blades.talent_enabled and not Config:GetSpec("ignore_demon_blades_gains") then
				...
				if TJ.lastMainhandAttack < now - mh_speed then
					TJ.lastMainhandAttack = now
				end
				if TJ.lastOffhandAttack < now - oh_speed then
					TJ.lastOffhandAttack = now
				end
		...
		curr = function(power, env)
			return power.sampled - power.spent + power.gained + power.gained_from_autoattacks
		end,
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Fury) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	soul_fragments = {
		...
		gained = 0,
		curr = function(power, env)
			return power.aura_stack - power.spent + power.gained
		end,
		can_spend = generic_can_spend,
		perform_spend = generic_perform_spend
	},
	rune = {
		...
		time_to_1 = function(power, env)
			return power.sample[1]
		end,
		time_to_2 = function(power, env)
			return power.sample[2]
		end,
		time_to_3 = function(power, env)
			return power.sample[3]
		end,
		time_to_4 = function(power, env)
			return power.sample[4]
		end,
		time_to_5 = function(power, env)
			return power.sample[5]
		end,
		time_to_6 = function(power, env)
			return power.sample[6]
		end,
		can_spend = generic_can_spend,
		perform_spend = generic_perform_spend
	},
	runic_power = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_RunicPower) or 0)
		end,
		gained = 0,
		spent = 0,
		curr = function(power, env)
			return power.sampled - power.spent + power.gained + ((env.rune.spent - env.rune.skipped) * 10)
		end,
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_RunicPower) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	maelstrom = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_Maelstrom) or 0)
		end,
		gained = 0,
		spent = 0,
		curr = function(power, env)
		...
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_Maelstrom) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
	...
	holy_power = {
		sampled = function(power, env)
			return (UnitPower("player", Enum_PowerType_HolyPower) or 0)
		end,
		gained = 0,
		spent = 0,
		curr = function(power, env)
		...
		max = function(power, env)
			return (UnitPowerMax("player", Enum_PowerType_HolyPower) or 0)
		end,
		deficit = function(power, env)
			return power.max - power.curr
		end,
