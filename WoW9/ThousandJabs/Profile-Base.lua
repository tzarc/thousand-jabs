...
local function expressionPrimaryModifier(keyword, profileSubstitutionsPre, profileSubstitutionsPost)
	...
	-- Casting checks
	if keyword == "debuff.casting.up" then
		keyword = "target.is_casting"
	end
	if keyword == "debuff.casting.down" then
		keyword = "(not target.is_casting)"
	end
	if keyword == "target.debuff.casting.up" then
		keyword = "target.is_casting"
	end
	if keyword == "target.debuff.casting.react" then
		keyword = "target.is_casting"
	end
	if keyword == "target.debuff.casting.down" then
		keyword = "(not target.is_casting)"
	end
	if keyword == "buff.casting.up" then
		keyword = "player.is_casting"
	end
	if keyword == "buff.casting.down" then
		keyword = "(not player.is_casting)"
	end

	-- GCD values
	if keyword == "gcd.max" then
		keyword = "gcd_max"
	end
	if keyword == "gcd.remains" then
		keyword = "gcd_remains"
	end

	-- Single-word conversions
	if keyword == "health" then
		keyword = "health.curr"
	end
	if keyword == "target" then
		keyword = "target.curr"
	end
	if keyword == "time_to_die" then
		keyword = "target.time_to_die"
	end
	if keyword == "refreshable" then
		keyword = "spell.THIS_SPELL.refreshable"
	end
	if keyword == "time" then
		keyword = "time_since_combat_start"
	end
	if keyword == "cooldown" then
		keyword = "cooldown.THIS_SPELL.remains"
	end
	if keyword == "cooldown_react" then
		keyword = "cooldown.THIS_SPELL.react"
	end
	if keyword == "duration" then
		keyword = "aura.THIS_SPELL.duration"
	end
	if keyword == "ticking" then
		keyword = "aura.THIS_SPELL.ticking"
	end
	if keyword == "delay" then
		keyword = "spell.THIS_SPELL.delay"
	end
	if keyword == "remains" then
		keyword = "aura.THIS_SPELL.remains"
	end
	if keyword == "cast_time" then
		keyword = "spell.THIS_SPELL.cast_time"
	end
	if keyword == "tick_time" then
		keyword = "spell.THIS_SPELL.tick_time"
	end
	if keyword == "charges" then
		keyword = "spell.THIS_SPELL.charges"
	end
	if keyword == "charges_fractional" then
		keyword = "spell.THIS_SPELL.charges_fractional"
	end
	if keyword == "max_charges" then
		keyword = "spell.THIS_SPELL.max_charges"
	end
	if keyword == "full_recharge_time" then
		keyword = "spell.THIS_SPELL.recharge_time"
	end -- TODO -- Check this?
	if keyword == "recharge_time" then
		keyword = "spell.THIS_SPELL.recharge_time"
	end
	if keyword == "execute_time" then
		keyword = "spell.THIS_SPELL.execute_time"
	end
	if keyword == "level" then
		keyword = "level.curr"
	end
	if keyword == "mana" then
		keyword = "mana.curr"
	end
	if keyword == "energy" then
		keyword = "energy.curr"
	end
	if keyword == "rage" then
		keyword = "rage.curr"
	end
	if keyword == "chi" then
		keyword = "chi.curr"
	end
	if keyword == "pain" then
		keyword = "pain.curr"
	end
	if keyword == "fury" then
		keyword = "fury.curr"
	end
	if keyword == "rune" then
		keyword = "rune.curr"
	end
	if keyword == "runic_power" then
		keyword = "runic_power.curr"
	end
	if keyword == "soul_fragments" then
		keyword = "soul_fragments.curr"
	end
	keyword = keyword:gsub("^soul_shard", "soul_shards")
	keyword = keyword:gsub("^soul_shardss", "soul_shards")
	if keyword == "soul_shards" then
		keyword = "soul_shards.curr"
	end
	if keyword == "maelstrom" then
		keyword = "maelstrom.curr"
	end
	if keyword == "holy_power" then
		keyword = "holy_power.curr"
	end
	...
	keyword =
		...
		function(a)
			return Core:Format("incoming_damage_over_%d", tonumber(a) * 1000)
		end
	)
	keyword =
		keyword:gsub(
		...
		function(a)
			return Core:Format("incoming_damage_over_%d", tonumber(a))
		end
	)
	...
	keyword =
		...
		"([%w_]+)%.([%w_]+)%.([%w_%.]+)",
		function(a, b, c)
			return Core:Format("%s.%s_%s", b, a, c:gsub("%.", "_"))
		end
	)
	...
	if type(action) == "table" then
		...
		if fullCooldownSecs and fullCooldownSecs > 0 then
			if isCooldownAffectedByHaste then
				action.CooldownTime = function(spell, env)
					return env.playerHasteMultiplier * fullCooldownSecs
				end
			else
				action.CooldownTime = fullCooldownSecs
			end

			action.spell_cooldown = function(spell, env)
			...
			action.perform_cast_funcsrc =
				action.perform_cast_funcsrc ..
				"; spell.cooldownStart = env.currentTime; spell.cooldownDuration = spell.CooldownTime"

			local cooldown_remains_override = rawget(action, "cooldown_remains_override")
			action.cooldown_remains =
				(type(cooldown_remains_override) ~= "nil" and cooldown_remains_override) or function(spell, env)
					return (spell.blacklisted and 999) or mmax(0, spell.cooldownStart + spell.cooldownDuration - env.currentTime)
				end
			action.cooldown_ready = function(spell, env)
				return (spell.cooldown_remains == 0) and true or false
			end
			action.cooldown_up = function(spell, env)
				return spell.cooldown_ready and true or false
			end
			action.cooldown_down = function(spell, env)
				return (not spell.cooldown_ready) and true or false
			end
			action.cooldown_react = function(spell, env)
				return spell.cooldown_ready and true or false
			end
			action.spell_charges = function(spell, env)
				return spell.cooldown_ready and 1 or 0
			end
			action.cooldown_full_recharge_time = function(spell, env)
				return spell.cooldown_remains and 1 or 0
			end
		end
	end
end
	...
	if type(action) == "table" then
		...
		if fullRechargeSecs and fullRechargeSecs > 0 then
			if isRechargeAffectedByHaste then
				action.RechargeTime = function(spell, env)
					return env.playerHasteMultiplier * fullRechargeSecs
				end
			else
			...
			if usesSpellCountForCharges then
				...
				action.spell_max_charges = function(spell, env)
					return spell.rechargeMax
				end
				action.spell_charges_fractional = function(spell, env)
					local f =
						(spell.rechargeSampled == spell.rechargeMax) and spell.rechargeMax - spell.rechargeSpent + spell.rechargeGained or
						spell.rechargeSampled + (env.currentTime - spell.rechargeStartTime) / spell.rechargeDuration - spell.rechargeSpent +
							spell.rechargeGained
			...
			action.cooldown_charges_fractional = function(spell, env)
				return spell.spell_charges_fractional
			end
			action.cooldown_charges = function(spell, env)
				return spell.spell_charges
			end
...
local function addActionAuraFields(action)
	if type(action) == "table" then
		if rawget(action, "AuraID") then
			...
			action.aura_up = rawget(action, "aura_up") or function(spell, env)
					return (spell.aura_remains > 0) and true or false
				end
			action.aura_down = rawget(action, "aura_down") or function(spell, env)
					return (not spell.aura_up) and true or false
				end
			action.aura_ticking = rawget(action, "aura_ticking") or function(spell, env)
					return spell.aura_up and true or false
				end
			action.aura_react = rawget(action, "aura_react") or function(spell, env)
					return spell.aura_up and true or false
				end
			action.aura_stack = rawget(action, "aura_stack") or function(spell, env)
					return spell.auraCount or 0
				end

			action.spell_tick_time = rawget(action, "spell_tick_time") or function(spell, env)
					return (spell.aura_up) and 1 or 0
				end -- TBD
			action.aura_tick_time_remains = rawget(action, "aura_tick_time_remains") or function(spell, env)
					return (spell.aura_up) and spell.spell_tick_time or 0
				end -- TBD
...
local function addMissingFields(action)
	if type(action) == "table" then
		if not rawget(action, "last_cast") then
			action.time_since_last_cast = 0
		end
		if not rawget(action, "time_since_last_cast") then
			action.time_since_last_cast = 99999
		end

		if not rawget(action, "spell_cast_time") then
			action.spell_cast_time = function(spell, env)
				return TJ.currentGCD
			end
		end
		if not rawget(action, "spell_execute_time") then -- TODO: Is this right?
			action.spell_execute_time = function(spell, env)
				return action.spell_cast_time(spell, env)
			end
		end

		if not rawget(action, "talent_enabled") then
			action.talent_enabled = false
		end

		if not rawget(action, "spell_recharge_time") then
			action.spell_recharge_time = 99999
		end
		if not rawget(action, "spell_cooldown") then
			action.spell_cooldown = 99999
		end

		if not rawget(action, "cooldown_remains") then
			local cooldown_remains_override = rawget(action, "cooldown_remains_override")
			action.cooldown_remains = (type(cooldown_remains_override) ~= "nil" and cooldown_remains_override) or 999999
		end
		if not rawget(action, "cooldown_ready") then
			action.cooldown_ready = false
		end
		if not rawget(action, "cooldown_up") then
			action.cooldown_up = false
		end
		if not rawget(action, "cooldown_down") then
			action.cooldown_down = false
		end
		if not rawget(action, "cooldown_react") then
			action.cooldown_react = false
		end
		if not rawget(action, "cooldown_charges") then
			action.cooldown_charges = 0
		end
		if not rawget(action, "cooldown_charges_fractional") then
			action.cooldown_charges_fractional = 0
		end

		if not rawget(action, "spell_charges") then
			action.spell_charges = 0
		end
		if not rawget(action, "spell_charges_fractional") then
			action.spell_charges_fractional = 0
		end
		if not rawget(action, "spell_max_charges") then
			action.spell_max_charges = 0
		end
		if not rawget(action, "spell_recharge_time") then
			action.spell_recharge_time = 0
		end

		if not rawget(action, "aura_remains") then
			action.aura_remains = 0
		end
		if not rawget(action, "aura_up") then
			action.aura_up = false
		end
		if not rawget(action, "aura_down") then
			action.aura_down = true
		end
		if not rawget(action, "aura_ticking") then
			action.aura_ticking = false
		end
		if not rawget(action, "aura_react") then
			action.aura_react = false
		end
		if not rawget(action, "aura_stack") then
			action.aura_stack = 0
		end
		...
		if rawget(action, "OverallSpellID") then
			...
			if not rawget(action, "Name") then
				action.Name = select(1, GetSpellInfo(overallSpellID))
			end
			if not rawget(action, "Icon") then
				action.Icon = select(3, GetSpellInfo(overallSpellID))
			end
		end
	end
end
...
function TJ:RegisterPlayerClass(config)
	...
	for k, v in pairs(config.resources) do
		resources[v] = Core.Environment.resources[v]
	end
	...
	function profile:ResetActions()
		if self.parsedActions then
			rt(self.parsedActions)
		end
	...
	function profile:LoadActions()
		if Core.devMode or not self.parsedActions then
			...
			local function primaryModifier(str)
				if converted[str] then
					return converted[str]
				end
				local res =
					expressionPrimaryModifier(str, config.conditional_substitutions_pre, config.conditional_substitutions_post)
				converted[str] = res
				return res
			end
			...
			if not self.parsedActions then
				...
				if aplDef.classID == config.class_id and aplDef.specID == config.spec_id then
					...
					Core:Error(
						Core:Format(
							"Mismatching class/spec IDs: APL expects %s/%s, character is %s/%s.",
							aplDef.classID,
							aplDef.specID,
							config.class_id,
							config.spec_id
			...
			for listName, listTable in pairs(self.parsedActions) do
				...
				for _, entry in pairs(listTable) do
					...
					entry.key =
						(entry.action == "run_action_list" or entry.action == "call_action_list") and
						Core:Format("%s:%s[%s]", listName, entry.action, entry.params.name) or
						Core:Format("%s:%s[%s]", listName, entry.action, counts[listName][entry.action])

					-- Create a default prcondition, based on whether the spell is castable
					entry.precondition =
						(entry.action == "run_action_list" or entry.action == "call_action_list") and "true" or
						Core:Format("%s.spell_can_cast", entry.action)
					if type(entry.params.sync) ~= "nil" then
						entry.precondition =
							Core:Format(
							"(%s) and (%s.spell_can_cast or %s.cooldown_up or %s.aura_remains > 0)",
							entry.precondition,
							entry.params.sync,
							entry.params.sync,
							entry.params.sync
					...
					if type(entry.condition_func) == "nil" then
						if type(entry.params.condition_converted) ~= "nil" or type(entry.params.target_if_converted) ~= "nil" then
							...
							local cond =
								(cond_if and cond_target_if and Core:Format("((%s) and (%s))", cond_if, cond_target_if)) or cond_if or
								cond_target_if or
								"false"
							entry.fullconditionfuncsrc =
								Core:Format("((%s) and (%s))", entry.precondition, cond:gsub("THIS_SPELL", entry.action))
						else
							entry.fullconditionfuncsrc = entry.precondition
						end
						entry.condition_func =
							Core:LoadFunctionString(
							Core:Format("function() return ((%s) and true or false) end", fixMathFuncs(entry.fullconditionfuncsrc)),
							Core:Format("cond:%s", entry.key)
						)
					end
					...
					if type(entry.value_func) == "nil" and type(entry.params.value_converted) ~= "nil" then
						...
						entry.value_func =
							Core:LoadFunctionString(
							Core:Format("function() return (%s) end", fixMathFuncs(entry.fullvaluefuncsrc)),
							Core:Format("var:%s", entry.key)
						)
					end
				end
			end
	...
	function profile:FindActionForSpellID(spellID)
		if self.spellIDactionMapping[spellID] then
			return self.spellIDactionMapping[spellID]
		end
	...
	function profile:Activate()
		...
		for k, v in pairs(actions) do
			...
			v.OverallSpellID =
				spellIDs and (type(spellIDs) == "table" and rawget(spellIDs, 1) or spellIDs) or
				auraIDs and (type(auraIDs) == "table" and rawget(auraIDs, 1) or auraIDs) or
				talentID and select(6, GetTalentInfoByID(talentID))
			...
			if type(v) == "table" and rawget(v, "AbilityID") then
				...
				if rawget(v, "SpellBookItem") then
					v.in_range = function(spell, env)
						if not v.in_spellbook then
							return false
						end
						local r =
							IsSpellInRange(spell.SpellBookItem, spell.SpellBookCaster == "pet" and BOOKTYPE_PET or BOOKTYPE_SPELL, "target")
				...
				v.spell_can_cast_funcsrc =
					Core:Format(
					"(not spell.blacklisted) and (env.player_level >= %d) and (spell.in_spellbook)",
					GetSpellLevelLearned(v.AbilityID)
				)
				...
				if costType then
					...
					v.spell_can_cast_funcsrc =
						v.spell_can_cast_funcsrc ..
						Core:Format(
							" and (env.%s.can_spend(env.%s, env, '%s', '%s', spell.%s_cost))",
							costType,
							costType,
							k,
							costType,
					...
					v.perform_cast_funcsrc =
						v.perform_cast_funcsrc ..
						Core:Format(
							"; env.%s.perform_spend(env.%s, env, '%s', '%s', spell.%s_cost)",
							costType,
							costType,
							k,
							costType,
				...
				local fullCooldownSecs =
					(isCooldownAffectedByHaste or false) and cooldownSecs / playerHasteMultiplier or cooldownSecs or 0
				...
				local fullRechargeSecs =
					(isRechargeAffectedByHaste or false) and rechargeSecs / playerHasteMultiplier or rechargeSecs or 0
				...
				if rawget(v, "AuraApplied") then
					v.perform_cast_funcsrc =
						v.perform_cast_funcsrc ..
						Core:Format("; env.%s.expirationTime = env.currentTime + %d", v.AuraApplied, v.AuraApplyLength)
				end
				...
				v.spell_can_cast_funcsrc =
					Core:Format(
					"function(spell, env) return ((%s) and true or false) end",
					v.spell_can_cast_funcsrc:gsub("^ and ", "")
				)
...
function TJ:GetAvailableProfilesForSpec(classID, specID)
	...
	local available =
		TJ.availableProfiles and TJ.availableProfiles[classID] and TJ.availableProfiles[classID][specID] or {}

	local defaultProfile =
		self.profiles and self.profiles[classID] and self.profiles[classID][specID] and
		self.profiles[classID][specID].defaultActionProfile
