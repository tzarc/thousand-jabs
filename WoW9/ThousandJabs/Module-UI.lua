	...
	frame.ReapplyLayout = function(self)
		...
		for i = 1, #childElements do
			...
			elseif e:IsVisible() then
				...
				if totalHeight < e:GetHeight() then
...
local function CreateSingleIconFrame(name, parent, sizeType, sizeIndex)
	...
	function button:Resize()
		local size =
			mceil(
			-0.001 + Config:Get("geometry", self.sizeType) * mpow(Config:Get("geometry", "sizeDecrease"), self.sizeIndex - 1)
		)
...
function UI:OnInitialize()
	...
	if MSQ then
		MSQ:Register(
			"ThousandJabs",
			function(_, group, skinID, gloss, backdrop, colours, isDisabled)
				if isDisabled then
...
function UI:SetAction(actionType, actionIndex, texture, overlayText)
	if not actionFrames.backdrop then
		return
	end
...
function UI:ReapplyLayout(skipMasque)
	...
	actionFrames.backdrop:SetPoint(
		Config:Get("position", "tgtPoint"),
		Config:Get("position", "offsetX"),
		Config:Get("position", "offsetY")
	)
...
function UI:CreateFrames()
	if actionFrames.baseFrame then
		return
	end

	-- Create the backdrop
	actionFrames.backdrop =
		CreateFrame("Frame", "ThousandJabsBackdrop", UIParent, BackdropTemplateMixin and "BackdropTemplate")
	actionFrames.backdrop:SetBackdropColor(0, 0, 0, Config:Get("backgroundOpacity"))
	...
	actionFrames.cooldown =
		CreateFrame(
		"Cooldown",
		"ThousandJabs_ST1Cooldown",
		actionFrames.containers[UI.SINGLE_TARGET],
