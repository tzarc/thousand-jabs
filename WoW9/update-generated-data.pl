...
our $minVersion = 90000;
our $maxVersion = 100000;
our $cachetime  = 86400;
our $verbose    = 0;
...
sub create_action_lists {
    ...
    for my $cls (sort keys %{$profiles}) {
        ...
        print {$outfile} "if GetBuildInfo and (select(4, GetBuildInfo()) < ${cfg::minVersion} or select(4, GetBuildInfo()) >= ${cfg::maxVersion}) then return end\n\n";
    ...
    for my $cls (sort keys %{$profiles}) {
        ...
        for my $spec (sort keys %{ $profiles->{$cls} }) {
            ...
            my $simc_cmd      = "'${simc::directory}/engine/simc' ${lcls}=${lcls}_${spec} default_actions=1 level=60 spec=${spec} ${mainhand} ${offhand} ${artifact} 'save=${new_simc_file}'";
