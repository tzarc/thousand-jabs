...
-- Library definition.
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local LibSandbox = LibStub:NewLibrary('LibTJSandbox-8.0', 1)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function LibSandbox:New(sandboxName, passThrough)
    -- Validate input...
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local function GetName(sandbox)
...
function LibSandbox:Get(sandboxInput)
    ...
    if not mt or not mt.getterObservers or not mt.setterObservers then
        error(('Sandbox "%s" has an invalid metatable.'):format(GetName(sandbox)))
    end
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function LibSandbox:Use(sandboxInput)
    -- Get the sandbox we're attempting to use
    local sandbox = self:Get(sandboxInput)

    -- Check that someone else hasn't already modified the environment
    local prevEnv = getfenv(2)
    if prevEnv ~= real_G then
        error(('Sandbox "%s" has another environment attached already.'):format(GetName(sandbox)))
    end
