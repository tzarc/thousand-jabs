...
actions+=/fury_of_air,if=!ticking&maelstrom>40
...
actions+=/crash_lightning,if=active_enemies>1|talent.crashing_storm.enabled|talent.boulderfist.enabled|feral_spirit.remains>5
...
actions+=/sundering
actions+=/lava_lash,if=(talent.fury_of_air.enabled&talent.overcharge.enabled&maelstrom>=120)|(!talent.overcharge.enabled&maelstrom>=80)
actions+=/rockbiter
actions+=/flametongue
actions+=/boulderfist
]]
...
actions+=/run_action_list,name=aoe,if=active_enemies>3&(spell_targets.chain_lightning>3|spell_targets.lava_beam>3)
...
actions.aoe+=/lava_burst,if=buff.lava_surge.up&spell_targets.chain_lightning=4
...
actions.single_asc+=/stormkeeper
...
actions.single_if+=/frost_shock,if=buff.icefury.up&((maelstrom>=20&raid_event.movement.in>buff.icefury.remains)|buff.icefury.remains<(1.5*spell_haste*buff.icefury.stack))
